...
/**
 * @title SonicTestnetWalletFactory
 ...
 */
contract SonicTestnetWalletFactory is Ownable {
    ...
    address public constant USDC = 0xA4879Fed32Ecbef99399e5cbC247E533421C4eC6; // Sonic Testnet
    ...
    /**
     * @notice Withdraw any native tokens (S) stuck in the factory (emergency only)
     */
    ...
    /**
     * @notice Fallback function to handle transactions with empty data
     * @dev This prevents transactions from reverting when data is stripped
     */
    fallback() external {
        // Allow the transaction to succeed even with empty data
        // This is needed because some RPC endpoints strip transaction data
    }
    
    /**
     * @notice Receive function to handle native token transfers
     * @dev This allows the factory to receive native tokens (S) if needed
     */
    receive() external payable {
        // Allow native tokens (S) to be sent to the factory
    }
}
...
contract BurnOnlyWallet {
    ...
    address public constant TOKEN_MESSENGER = 0x8FE6B999Dc680CcFDD5Bf7EB0974218be2542DAA;
    address public constant USDC = 0xA4879Fed32Ecbef99399e5cbC247E533421C4eC6;
    ...
    constructor(
        ...
        // Note: USDC approval will be handled when needed, not in constructor
    }
    ...
    /**
     * @notice Approve USDC spending for TokenMessenger (called before burning)
     * @dev This function must be called before burning USDC
     */
    ...
    // NO fallback or receive functions - cannot receive native tokens
}
